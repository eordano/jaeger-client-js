...
export class ConfigurationEnv {
  static _validateEnv() {
    Object.keys(deprecatedEnvVars).forEach(env => {
      if (process.env[env]) {
        console.warn(
          `You are using deprecated env variable ${env}. Use ${deprecatedEnvVars[env]} instead. \nDeprecated env variable will be removed in the next major release (4.x.x)`,
        )
      }
    })
  }
  ...
  static _getSamplerFromEnv(config) {
    ...
    let value = ConfigurationEnv._getConfigValue(
      config.sampler,
      'type',
      process.env.JAEGER_SAMPLER_TYPE,
    )
    if (value) {
    ...
    value = ConfigurationEnv._getConfigValue(
      config.sampler,
      'param',
      process.env.JAEGER_SAMPLER_PARAM,
    )
    if (value) {
    ...
    value = ConfigurationEnv._getConfigValue(
      config.sampler,
      'host',
      process.env.JAEGER_SAMPLER_HOST,
    )
    if (value) {
    ...
    value = ConfigurationEnv._getConfigValue(
      config.sampler,
      'port',
      process.env.JAEGER_SAMPLER_PORT,
    )
    if (value) {
