...
import { BaggageRestrictionManager } from '../types/baggage_restriction_manager'

/**
 * BaggageSetter is a class that sets a baggage key:value and the associated
 * logs on a Span.
 */
export class BaggageSetter {
  ...
  setBaggage(span: Span, key: string, baggageValue: string): SpanContext {
    ...
    let truncated = false
    let prevItem = ''
    let restriction = this._restrictionManager.getRestriction(
      span.serviceName,
      key,
    )
