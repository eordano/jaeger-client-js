...
export class Configuration {
  static _getSampler(config, options) {
    ...
    if (typeof param !== 'number') {
      throw new Error(
        `Expecting sampler.param to be a number. Received ${param}`,
      )
    }
  ...
  static _getReporter(config, options) {
    ...
    let sender = isHTTPSender
      ? new HTTPSender(senderConfig)
      : new UDPSender(senderConfig)
  ...
  static initTracer(config: Record<string, any>, options: Record<string, any> = {}) {
    ...
    if (options.logger) {
      options.logger.info(
        `Initializing Jaeger Tracer with ${reporter} and ${sampler}`,
      )
    }
