...
export class RemoteControlledSampler implements Sampler {
  ...
  constructor(serviceName: string, options: any = {}) {
    ...
    this._sampler = adaptSamplerOrThrow(
      options.sampler ||
        new ProbabilisticSampler(DEFAULT_INITIAL_SAMPLING_RATE),
    )
  ...
  _updateSampler(response: SamplingStrategyResponse): boolean {
    if (response.operationSampling) {
      ...
      this._sampler = new PerOperationSampler(
    ...
    if (
      response.strategyType === PROBABILISTIC_STRATEGY_TYPE &&
    ...
    if (
      response.strategyType === RATE_LIMITING_STRATEGY_TYPE &&
