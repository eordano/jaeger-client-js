...
export class GuaranteedThroughputSampler implements LegacySamplerV1 {
  ...
  toString(): string {
    return `${this.name()}(samplingRate=${
      this._probabilisticSampler.samplingRate
    }, lowerBound=${this._lowerBoundSampler.maxTracesPerSecond})`
  }
  ...
  isSampled(operation: string, tags: any): boolean {
    ...
    let decision = this._lowerBoundSampler.isSampled(
      operation,
      this._tagsPlaceholder,
    )
    if (decision) {
      ...
      tags[
        constants.SAMPLER_PARAM_TAG_KEY
      ] = this._probabilisticSampler.samplingRate
    }
    return decision
  }
