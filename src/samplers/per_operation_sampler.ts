...
type SamplersByOperation = {
  __proto__: null
  [key: string]: GuaranteedThroughputSampler
}
...
export class PerOperationSampler implements Sampler {
  ...
  update(strategies: PerOperationSamplingStrategies): boolean {
    ...
    if (
      ...
      this._defaultSampler.samplingRate != defaultSamplingRate
    ) {
  ...
  isSampled(operation: string, tags: any): boolean {
    ...
    if (!sampler) {
      if (
        Object.keys(this._samplersByOperation).length >= this._maxOperations
      ) {
