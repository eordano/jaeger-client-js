...
export class PrometheusMetricsFactory {
  ...
  _createMetric(metric: any, name: string, labels: {}): any {
    ...
    return labelValues.length > 0
      ? this._cache[key].labels(...labelValues)
      : this._cache[key]
  }
  ...
  createCounter(name: string, tags: {}): Counter {
    return new CounterPromWrapper(
      this._createMetric(this._promClient.Counter, name, tags),
    )
  }
  ...
  createGauge(name: string, tags: {}): Gauge {
    return new GaugePromWrapper(
      this._createMetric(this._promClient.Gauge, name, tags),
    )
  }
}
