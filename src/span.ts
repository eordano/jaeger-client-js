...
export class Span {
  ...
  setBaggageItem(key: string, value: string): Span {
    ...
    this._spanContext = this._baggageSetter.setBaggage(
      this,
      normalizedKey,
      value,
    )
  ...
  setOperationName(operationName: string): Span {
    ...
    if (!this._spanContext.samplingFinalized) {
      const decision = this.tracer()._sampler.onSetOperationName(
        this,
        operationName,
      )
   ...
   * @return {Span} - returns this span.
   */
  log(keyValuePairs: any, timestamp?: number | null): Span {
    if (this._isWriteable()) {
      this._logs.push({
